Ext.define('FHD.view.kpi.myfolder.MyKpiGrid', {
    extend: 'FHD.ux.kpi.KpiGridPanel',
    alias: 'widget.mykpigrid',
    /**
     * 编辑指标
     */
    kpiEditFun:function(){
    ...
    addListerner: function () {
        ...
        me.on('selectionchange', function () {
            if (me.down('#mykpi_kpiedit' )) {
                me.down('#mykpi_kpiedit' ).setDisabled(me.getSelectionModel().getSelection().length === 0);
            }
            if (me.down('#mykpi_kpidel' )) {
                me.down('#mykpi_kpidel' ).setDisabled(me.getSelectionModel().getSelection().length === 0);
            }
            if(me.down('#mykpi_disable')){
            	me.down('#mykpi_disable').setDisabled(me.getSelectionModel().getSelection().length === 0);
            }
            if(me.down('#mykpi_enable')){
            	me.down('#mykpi_enable').setDisabled(me.getSelectionModel().getSelection().length === 0);
            }
