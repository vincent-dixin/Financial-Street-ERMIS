 ...
 * @author 陈晓哲
 */
Ext.define('FHD.view.kpi.scorecard.ScorecardWarningSet', {
    extend: 'FHD.ux.EditorGridPanel',
    alias: 'widget.scorecardwarningset',
                ...
                callback: function (data) {
                    if (data && data.success) {
                        ...
                        if(!finishflag){//如果点击的不是完成按钮,需要移动到下一个面板
                        	...
                        	scorecardbasicinfopanel.navBtnState(cardPanel);//设置导航按钮状态,如果是首个面板则上一步按钮为置灰状态,如果是最后一个面板则下一步按钮为置灰状态
                        }else{
    ...
    initComponent: function () {
        ...
        me.on('selectionchange', function(){
        	//删除按钮
        	if(me.down('#categorydelBtn')){
        		me.down('#categorydelBtn').setDisabled(me.getSelectionModel().getSelection().length === 0);
        	}
        }); 
    ...
    reLoadData: function () {
    	...
    	var scorecardtab = Ext.getCmp('scorecardtab');
    	//加载告警列表数据
    	me.store.proxy.extraParams.id = scorecardtab.paramObj.categoryid;
    	me.store.proxy.extraParams.editflag = scorecardtab.paramObj.editflag;
		me.store.load();
    }
