...
Ext.define('FHD.view.comm.report.assess.GroupYearReportList', {
	...
	requires: [
		'FHD.view.comm.report.assess.GroupYearReportForm'
	],
	...
	initComponent: function(){
		...
		Ext.apply(me,{
			tbarItems : [{
				iconCls: 'icon-add',
				text: '添加',
				id:'group_year_assessment_report_add',
				tooltip: '添加公司年度评价报告',
				handler : me.onSave
			},{
				iconCls: 'icon-edit',
				text: '修改',
				id:'group_year_assessment_report_edit',
				tooltip: '添加公司年度评价报告',
				handler : me.onEdit
			},{
				iconCls: 'icon-del',
				text: '删除',
				id:'group_year_assessment_report_del',
				tooltip: '删除公司年度评价报告',
				handler : me.onDel
			}]
		});
	...
	setstatus: function(){
        ...
        me.down('#group_year_assessment_report_del').setDisabled(length === 0);
        if(length != 1){
        	me.down('#group_year_assessment_report_edit').setDisabled(true);
        }else{
        	me.down('#group_year_assessment_report_edit').setDisabled(false);
        }
    },
	...
	onSave : function(){
		...
		assessplancenterpanel.add(Ext.widget('groupyearreportform'));
	},
	...
	onEdit : function() {
		...
		var assessplancenterpanel = this.up('assessplancenterpanel');
		assessplancenterpanel.removeAll(true);
		var groupyearreportform = Ext.widget('groupyearreportform',{reportId:selection.get('id')});
