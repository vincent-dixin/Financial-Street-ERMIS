...
Ext.define('FHD.ux.org.CompSelect', {
    extend:'Ext.form.field.ComboBox',
    requires: ['Ext.selection.Model', 'Ext.data.Store'],
	alias : 'widget.compselect',
    ...
    /**
     * @cfg {Number} minHeight The minimum height to allow when <tt>{@link Ext.form.field.Text#grow grow}=true</tt>
     * (defaults to <tt>false</tt>, which allows for natural growth based on selections)
     */
    minHeight: false,

    /**
     * @cfg {Number} maxHeight The maximum height to allow when <tt>{@link Ext.form.field.Text#grow grow}=true</tt>
     * (defaults to <tt>false</tt>, which allows for natural growth based on selections)
     */
    maxHeight: 50,
    ...
    initComponent: function() {
        ...
        me.bindStore(Ext.create('Ext.data.Store', {
		    model:'CompSelectModel',
			proxy : {
				type : 'ajax',
				url : __ctxPath + '/sys/org/findCompanyByParentId.f?parentId=' + __user.companyId,
				reader : {
					type : 'json',
					root : 'companys'
				}
    ...
    afterRender: function() {
        ...
        if (me.grow) {
        	if(Ext.isIE){
        		if(me.multiSelect){
        			if (Ext.isNumber(me.maxHeight) && (me.maxHeight > 0)) {
        				me.itemList.applyStyles('height:'+me.maxHeight+'px;margin:0px;');
        			}
            	}else{
            		me.itemList.applyStyles('height:'+22+'px;margin:0px;overflow:hidden;');
            	}
        	}else{
        		if (Ext.isNumber(me.minHeight) && (me.minHeight > 0)) {
	                me.itemList.applyStyles('min-height:'+me.minHeight+'px');
	            }else{
	            	if(me.multiSelect){
	            		me.itemList.applyStyles('min-height:'+me.maxHeight+'px');
	            	}
	            }
	            if (Ext.isNumber(me.maxHeight) && (me.maxHeight > 0)) {
	                me.itemList.applyStyles('max-height:'+me.maxHeight+'px');
	            }
        	}
        }
