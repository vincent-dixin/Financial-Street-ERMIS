 ...
 * @author 陈晓哲
 */
Ext.define('Ext.kpi.kpi.opt.kpiTypeWarningPanel', {
                ...
                callback: function (data) {
                    if (data && data.success) {
                        ...
                        if(!finishflag){//如果点击的不是完成按钮,需要移动到下一个面板
                        	...
                        	/*同时将权限按钮为可用状态*/
                            Ext.getCmp('authority_btn'+ param_dc).setDisabled(false);
                            Ext.getCmp('authority_btn_top'+ param_dc).setDisabled(false);
                        }else{
    ...
    _addListerner:function(){
        ...
        me.on('selectionchange', function(){
        	//删除按钮
        	if(me.down('#kpitypedelBtn')){
        		me.down('#kpitypedelBtn').setDisabled(me.getSelectionModel().getSelection().length === 0);
        	}
        }); //选择记录发生改变时改变按钮可用状态
