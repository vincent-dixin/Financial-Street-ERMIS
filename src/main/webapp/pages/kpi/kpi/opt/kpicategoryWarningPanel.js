 ...
 * @author 陈晓哲
 */
Ext.define('Ext.kpi.kpi.opt.kpicategoryWarningPanel', {
                ...
                callback: function (data) {
                    if (data && data.success) {
                        ...
                        if(!finishflag){//如果点击的不是完成按钮,需要移动到下一个面板
                            ...
                            /*同时将权限按钮为可用状态*/
                            Ext.getCmp('kpicategory_authority_btn'+kpicategory_paramdc).setDisabled(false);
                            Ext.getCmp('kpicategory_authority_btn_top'+kpicategory_paramdc).setDisabled(false);
                        }else{
    ...
    _onchange:function(){
    	var grid = this;
    	//选择记录发生改变时改删除变按钮可用状态
    	if(grid.down('#kpicategory_delBtn'+kpicategory_paramdc)){
    		grid.down('#kpicategory_delBtn'+kpicategory_paramdc).setDisabled(grid.getSelectionModel().getSelection().length === 0);
    	}
    },
    /**
    ...
    _addListerner:function(){
    	var me = this;
        me.grid.on('selectionchange', me._onchange); //选择记录发生改变时改变按钮可用状态
    },
    /**
     * 初始化组件方法
     */
    initComponent: function () {
        ...
        me.grid = Ext.create('FHD.ux.EditorGridPanel', {
            ...
            tbarItems: [{
                id: 'kpicategory_addBtn'+kpicategory_paramdc,
                iconCls: 'icon-add',
                handler: function () {
                    me.addWarning();//添加记录
                },
                ...
                id: 'kpicategory_delBtn'+kpicategory_paramdc,
                iconCls: 'icon-del',
                handler: function () {
                    me.warningDel()//删除记录
                },
